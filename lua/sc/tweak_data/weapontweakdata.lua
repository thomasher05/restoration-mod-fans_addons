...
Hooks:PostHook( WeaponTweakData, "init", "SC_weapons", function(self)
			...
			--[[     HEAVY SHOTGUNS     ]]
				...
				--PRIMARIES
					...
					--Reinfeld 88 (WHY IS THIS UNDER THE REMINGTON LABEL)
						...
						self.m1897.BURST_FIRE_RATE_MULTIPLIER = 1.8
	...
	--[[     CUSTOM BEARDLIB WEAPONS     ]]--
		...
		--[[     PAWCIO'S MODS     ]]--
			...
			if self.lewis then --Pawcio's Lewis Gun
				...
				self.lewis.kick = self.stat_info.kick_tables.horizontal_recoil_mg
			...
			if self.super then --Pawcio's DOOM Super Shotgun
				...
				self.super.desc_id = "bm_super_sc_desc"
		...
		--[[     RJC9000'S MODS     ]]--
			...
			if self.stango44 then --"STG-44" (FUCK YOU SLEDGEHAMMER)
				...
				self.stango44.kick_pattern = {
					...
					{ 7, self.stat_info.kick_tables.pattern_v1 },
					{ 10, self.stat_info.kick_tables.pattern_l1 },
					{ 13, self.stat_info.kick_tables.pattern_v2 },
					{ 17, self.stat_info.kick_tables.pattern_r1 },
					{ 19, self.stat_info.kick_tables.pattern_r2 },
					{ 22, self.stat_info.kick_tables.pattern_r3 },
					{ 25, self.stat_info.kick_tables.pattern_r5 },
					{ 26, self.stat_info.kick_tables.pattern_r3 },
					{ 27, self.stat_info.kick_tables.pattern_r5 },
					{ 35, self.stat_info.kick_tables.pattern_r2 },
					{ 38, self.stat_info.kick_tables.pattern_r1 },
					{ 40, self.stat_info.kick_tables.right_kick },
				}
		...
		--[[     HYLIE'S MODS     ]]--
			...
			if self.mx63 then
				...
				self.mx63.kick = {}
				self.mx63.kick = self.stat_info.kick_tables.horizontal_recoil_mg
			...
			if self.bulldog then
				...
				self.bulldog.kick_pattern = {
					...
					{ 14, self.stat_info.kick_tables.horizontal_recoil_mg },
				}
		...
		--Serious Sam Pack
			if self.xm214a then
				...
				self.xm214a.has_description = true
				self.xm214a.desc_id = "bm_wpn_fps_lmg_xm214a_desc"
		...
		if self.thirdeye then --Third Eye Railgun
			self.thirdeye.CLIP_AMMO_MAX = 10
			self.thirdeye.AMMO_MAX = 30
			self.thirdeye.can_shoot_through_enemy = true
			self.thirdeye.can_shoot_through_shield = true
			self.thirdeye.can_shoot_through_wall = true
			self.thirdeye.can_shoot_through_titan_shield = true
			self.thirdeye.kick = self.stat_info.kick_tables.left_recoil
			self.thirdeye.supported = true
			self.thirdeye.ads_speed = 0.400
			self.thirdeye.AMMO_PICKUP = 2
			self.thirdeye.damage_falloff = {
				start_dist = 3000,
				end_dist = 6400,
				min_mult = 0.2
			}
	...
	--[[     CAP/WEAPONLIB REQUIRING THINGS     ]]	
		...
		if self.sideload then --UTS-15
			self.sideload.recategorize = { "light_shot" }
			self.sideload.categories = { "shotgun" }
			self.sideload.damage_type = "shotgun"
			...
			self.sideload.FIRE_MODE = "single"
			self.sideload.CAN_TOGGLE_FIREMODE = false
			self.sideload.rays = 10
			self.sideload.supported = true
			self.sideload.ads_speed = 0.420
			self.sideload.damage_falloff = {
				start_dist = 500,
				end_dist = 2800,
				min_mult = 0.15
			}
			...
			self.sideload.sounds.fire = "sideload_fire"
			self.sideload.sounds.fire_single = "sideload_fire"
			self.sideload.stats_modifiers = nil
			self.sideload.panic_suppression_chance = 0.05
			self.sideload.fire_anim_offset2 = 0.03
